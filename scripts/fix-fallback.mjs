...
const filePath = path.join(__dir, 'generate-lessons.mjs');

let content = readFileSync(filePath, 'utf8');

// New quiz with simple, ESL-friendly questions
const newQuiz = `quiz: [
      {
...
// New body with simple, beginner-friendly content
const newBody = `body: \`## \${title}
...
// Replace old quiz with new quiz
const quizRegex = /quiz: \[\s*\{[\s\S]*?\},\s*\],/;
if (quizRegex.test(content)) {
  content = content.replace(quizRegex, newQuiz);
  console.log('✅ Updated quiz array');
} else {
  console.log('❌ Could not find quiz array');
}

// Replace old body with new body
const bodyRegex = /body: `## \$\{title\}[\s\S]*?Use the quiz above to check retention, then continue with the next level lesson on ieltscorner\.ca\.`/;
if (bodyRegex.test(content)) {
  content = content.replace(bodyRegex, newBody);
  console.log('✅ Updated body content');
} else {
