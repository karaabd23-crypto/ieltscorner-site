...
async function getSubscriptionDetails(stripe, subscriptionRef) {
  ...
  if (!hasPrice) {
    return {
      valid: false,
      reason: `Subscription does not include the required ${PTE_CORE_PREMIUM_PRODUCT_NAME} price.`,
    };
  }
  ...
  try {
    ...
    if (session.mode !== 'subscription') {
      return {
        ...
        body: JSON.stringify({
          valid: false,
          reason: `This checkout session is not a ${PTE_CORE_PREMIUM_BILLING_INTERVAL}ly subscription for ${PTE_CORE_PREMIUM_PRODUCT_NAME}.`,
        }),
      };
    }
